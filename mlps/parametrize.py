...
features_input = ["age_Myr","Mini","logTe","label"]
photometric_bands = ['Gmag', 'G_BPmag', 'G_RPmag', 'Jmag', 'Hmag', 'Ksmag', 'gP1mag', 'rP1mag', 'iP1mag', 'zP1mag', 'yP1mag']
...
for name,df_tmp in dfg.__iter__():
	...
	spline,par = make_splprep(raw_data.T,k=4,s=1e-7)
	par[0] = min_par
	...
	if do_plots:
		mass,teff = spline(np.linspace(0.,1.,100))
		fig, ax = plt.subplots()
